...
const BarcodeScanner: React.FC<BarcodeScannerProps> = ({ onScan }) => {
  ...
  const instanceIdRef = useRef<string>(`scanner-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`);
  ...
  const {
    ...
    key: instanceIdRef.current // Use unique ID for this scanner instance
  });
  ...
  }, [stopScanning, resetScannerState]);
  ...
  const handleReset = () => {
    ...
    // Reset scanner state from hook before toggling
    resetScannerState();
    
    // Add a slight delay before toggling to ensure cleanup is complete
    setTimeout(() => {
      toggleScanning();
    }, 800); // Increased from 500ms to 800ms for more thorough cleanup
  };
